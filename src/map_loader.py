...
def load_next_map_zone_centers(zone_centers_filename, base_dir):
    ...
    zone_centers = np.vstack(
        (
            ...
            * np.expand_dims(np.arange(tm_config.n_zone_centers_extrapolate_before_start_of_map, 0, -1), axis=1),
            zone_centers,
            zone_centers[-1]
            + np.expand_dims(zone_centers[-1] - zone_centers[-2], axis=0)
            * np.expand_dims(np.arange(1, 1 + tm_config.n_zone_centers_extrapolate_after_end_of_map, 1), axis=1),
        )
    )
...
def map_name_from_map_path(map_path):
    gbx = Gbx(str(tm_config.trackmania_base_path / "Tracks" / "Challenges" / Path(map_path.strip("'\""))))
    gbx_challenge = gbx.get_class_by_id(GbxType.CHALLENGE)
    return gbx_challenge.map_name


def replay_personal_record(map_path):
    trackmania_replay_username = tm_config.username if config_copy.is_linux else os.getlogin()
    filename = trackmania_replay_username + "_" + map_name_from_map_path(map_path) + ".Replay.gbx"
    filepath = tm_config.trackmania_base_path / "Tracks" / "Replays" / "Autosaves"
...
def get_checkpoint_positions_from_gbx(map_path: str):
    ...
    """
    g = Gbx(str(tm_config.trackmania_base_path / "Tracks" / "Challenges" / map_path.strip("'\"").replace("\\", "/")))
...
def sync_virtual_and_real_checkpoints(zone_centers: npt.NDArray, map_path: str):
    ...
    if tm_config.sync_virtual_and_real_checkpoints:
...
def analyze_map_cycle(map_cycle):
    ...
    """
    set_all_maps = set(map(lambda x: x[0], (a for a in itertools.chain(*copy.deepcopy(tm_config.map_cycle)))))
