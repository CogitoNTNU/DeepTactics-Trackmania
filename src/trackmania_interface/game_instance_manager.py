...
class GameInstanceManager:
    ...
    def rollout(self, exploration_policy: Callable, map_path: str, zone_centers: npt.NDArray, update_network: Callable):
        ...
        if time.perf_counter() - self.last_game_reboot > tm_config.game_reboot_interval:
            self.close_game()
            self.iface = None
            self.launch_game()
        ...
        _time = -3000
        current_zone_idx = tm_config.n_zone_centers_extrapolate_before_start_of_map
        ...
        try:
            while not this_rollout_is_finished:
                if compute_action_asap_floats:
                    ...
                    sim_state_car_gear_and_wheels = np.array(
                        [
                            ...
                            *(
                                ...
                                for i in range(tm_config.n_contact_material_physics_behavior_types)
                            ),
                        ],
                        dtype=np.float32,
                    )
                    if sim_state_position[1] > tm_config.deck_height:
                        current_zone_idx = update_current_zone_idx(
                            current_zone_idx,
                            zone_centers,
                            sim_state_position,
                            tm_config.max_allowable_distance_to_virtual_checkpoint,
                            self.next_real_checkpoint_positions,
                            self.max_allowable_distance_to_real_checkpoint,
                        )
                    ...
                    state_zone_center_coordinates_in_car_reference_system = sim_state_orientation.dot(
                        (
                            zone_centers[
                                ...
                                + tm_config.one_every_n_zone_centers_in_inputs
                                * tm_config.n_zone_centers_in_inputs : tm_config.one_every_n_zone_centers_in_inputs,
                                :,
                            ]
                            - sim_state_position
                        ).T
                    ...
                    previous_actions = [
                        tm_config.inputs[rollout_results["actions"][k] if k >= 0 else tm_config.action_forward_idx]
                        for k in range(
                            len(rollout_results["actions"]) - tm_config.n_prev_actions_in_inputs, len(rollout_results["actions"])
                        )
                    ]
                    ...
                    floats = np.hstack(
                        (
                            ...
                            min(
                                tm_config.margin_to_announce_finish_meters,
                                distance_from_start_track_to_prev_zone_transition[
                                    len(zone_centers) - tm_config.n_zone_centers_extrapolate_after_end_of_map
                                ]
                                - distance_since_track_begin,
                            ),
                ...
                if msgtype == int(MessageType.SC_RUN_STEP_SYNC):
                    ...
                    if (not give_up_signal_has_been_sent) and (map_path != self.latest_map_path_requested):
                        ...
                        end_race_stats["race_time"] = tm_config.cutoff_rollout_if_race_not_finished_within_duration_ms
                        ...
                        self.iface.set_timeout(tm_config.timeout_between_runs_ms)
                        if frame_expected:
                    ...
                    if not this_rollout_is_finished:
                        ...
                        if _time >= 0 and _time % (10 * self.run_steps_per_action) == 0 and this_rollout_has_seen_t_negative:
                            ...
                            if _time % (10 * self.run_steps_per_action * tm_config.update_inference_network_every_n_actions) == 0:
                                update_network()
                    ...
                    if (
                        ...
                        rollout_results["input_w"].append(tm_config.inputs[action_idx]["accelerate"])
                    ...
                    self.iface.execute_command(f"set autologin {tm_config.username}")
